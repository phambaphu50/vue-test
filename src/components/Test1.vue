    ...
    <table class="table ui-accordion">
     ...
     <tbody>
        <template v-for="(row, index) in data.rows">
          <tr class="head" :key="row.title.order" v-on:click="loadContent(index)">
          ...
          <tr
            class="body"
            :class="{ active: productIndexs.includes(index)}"
            :key="`${row.title.order}_content}`"
          >
            <td colspan="3">
              <div class="answer img-preview" id="cont0" v-html="row.content"></div>
            </td>
          </tr>
        </template>
      </tbody>
...
export default {
  ...
  methods: {
    loadContent: function(index) {
      if (this.productIndexs.includes(index)) {
        this.productIndexs = [];
      } else {
        this.productIndexs.splice(0, 1);
        this.productIndexs.push(index);
      }
    }
  }
};
...
.table-wrap.coltype table tbody tr:first-child th,
.table-wrap.coltype table tbody tr:first-child td {
